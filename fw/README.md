...
This folder contains the firmware for the RocSync device. The project uses [PlatformIO](https://platformio.org/) with [ch32fun](https://github.com/cnlohr/ch32fun) as the development environment. Previous versions used the Arduino core for CH32V003 but have been migrated to ch32fun for reduced flash memory usage. Use the WCH-LinkE USB debugger or any compatible programmer to flash the firmware. Both hardware revisions share the same source tree, with the revision selected at compile time via preprocessor directive (`#define REV1` or `#define REV2`). Counter brightness is set to 50% to improve Atracsys fTk fiducial detection; at full brightness, nearby LEDs were sometimes detected as one fiducial (TODO: verify if still necessary for Rev2 with increased LED spacing).


## Features

- Configurable brightness via PWM
- Standby mode that reduces power consumption by keeping the timer running but disabling LEDs
- (Rev1) Single button to toggle standby mode; brightness is fixed at compile time
- (Rev2) 5-way buttons for user input. Up/down adjusts counter brightness, left/right adjust ring brightness, and center toggles standby mode. (TODO: implement non-volatile storage for brightness settings)
